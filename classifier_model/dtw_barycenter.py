...
def compute_dtw_dists(correct_sample_list: list, wrong_sample_list: list, barycenters: list, ebars: list,
                      method=[2, 5]):
    """
    ...
    for i in correct_sample_list:
        px, py = execute_method(i, barycenters, ebars, method)
        correct_points[0].extend(px)
        correct_points[1].extend(py)
    for i in wrong_sample_list:
        px, py = execute_method(i, barycenters, ebars, method)
        wrong_points[0].extend(px)
        wrong_points[1].extend(py)
...
def execute_method(signal, comparrison_set1, comparrison_set2, method=[2, 5]):
    ...
    for j in range(len(comparrison_set1)):
        ...
        if comparrison_set2:
            ...
            dist2 = 0
    points[0].append(dist1)
    points[1].append(dist2)
    return points
...
def return_relative_distance_of_barycenters(barycenters, method=[2, 5]):
    ...
    for i in range(len(barycenters)):
        for j in range(i, len(barycenters)):
            bar[0].append(get_distance(barycenters[i], barycenters[j], method[0]))
            bar[1].append(get_distance(barycenters[i], barycenters[j], method[1]))
    return bar
